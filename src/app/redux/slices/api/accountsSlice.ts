...
export const createAccount = createAsyncThunk(
  ...
  async (accountData: Account, { dispatch, rejectWithValue }) => {
    try {
      await axios.post(`${getStagingApi()}/account/create`, accountData, {
        headers: {
...
export const fetchAccounts = createAsyncThunk(
  ...
  async (userId: string, { rejectWithValue }) => {
    try {
      const response = await axios.get(`${getStagingApi()}/account/all?userId=${userId}`);
  ...
  async ({ name, userId }: { name: string; userId: string }, { rejectWithValue }) => {
    try {
      const response = await axios.get(
        `${getStagingApi()}/account/search?name=${name}&userId=${userId}`,
        {
          headers: {
            "Content-Type": "application/json",
          },
