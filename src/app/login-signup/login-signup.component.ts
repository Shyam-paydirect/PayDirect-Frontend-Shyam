...
export class LoginSignupComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    bullets.forEach((bullet: Element) => {
      ...
      this.renderer.listen(bulletElement, "click", () => this.moveSlider(bulletElement, images, bullets));
    });
  }

  private moveSlider(bullet: HTMLElement, images: NodeListOf<Element>, bullets: NodeListOf<Element>) {
    ...
    const textSlider = this.el.nativeElement.querySelector(".text-group") as HTMLElement;
    if (textSlider) {
      this.renderer.setStyle(textSlider, "transform", `translateY(${-(Number(index) - 1) * 2.2}rem)`);
    }
