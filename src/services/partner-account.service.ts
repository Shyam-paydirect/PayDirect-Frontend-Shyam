...
class PartnerAccountService {
  ...
  private transformToNewFormat(accountData: PartnerAccountData, accountId: string = 'acct_1234567890'): NewPartnerData {
    return {
  ...
  async createPartnerAccount(accountData: PartnerAccountData & { account_id?: string }): Promise<PartnerAccountResponse> {
    try {
      ...
      const accountId = accountData.account_id || 'acct_1234567890';
